exports.run = async({ client, message, args, lang, prefix }) => {
    ...
    if (!args.join(" ")) return message.reply({ embeds: [new client.utils.embed().setDescription(lang.say.use.replace("+prefix+", prefix))] })//message.reply(await client.utils.lang(lang, 'digito'));
    let say = args.join(' ');
