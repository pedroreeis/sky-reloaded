module.exports.run = async ({ client, channel, message, author, lang }) => {
    ...
    if (userRank[0] == null) return message.reply({ embeds: [new client.utils.embed().setDescription(lang.rank.faltaLvl("+author+", author))]})
    ...
    channel.send({ files: [new client.utils.attach(rank, 'est.png')]})

    /*client.utils.mysql.query(`SELECT * FROM users WHERE ID = '${author.id}' && guild = '${message.guild.id}'`, async(err, rowsAll) => {

        if (rowsAll[0] == null) return message.reply(new client.utils.embed().setDescription(`${await client.utils.lang(lang, 'member')} ${author} ${await client.utils.lang(lang, 'level')}`))
